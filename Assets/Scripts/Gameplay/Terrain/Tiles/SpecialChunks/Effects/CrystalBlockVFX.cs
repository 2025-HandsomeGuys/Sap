// @tags: vfx, audio, special-chunk, digging, particle, crystal
using UnityEngine;

/// <summary>
/// 수정 블록의 파티클·사운드 효과를 담당하는 컴포넌트.
///
/// SOLID — SRP: VFX/오디오만. HP 판정·드롭·비주얼 스프라이트와 완전 분리.
/// CrystalBlockEntity가 PlayHit() / PlayDestroy()로 직접 호출.
/// </summary>
[RequireComponent(typeof(AudioSource))]
public class CrystalBlockVFX : MonoBehaviour
{
    [Header("타격 이펙트")]
    [Tooltip("타격 시 재생할 수정 파편 파티클 (없으면 생략)")]
    ...
    /// <summary>타격마다 호출. CrystalBlockEntity.Dig()에서 사용.</summary>
    public void PlayHit()
    {
    ...
    /// <summary>파괴 직전 호출. CrystalBlockEntity.Die()에서 사용.</summary>
