...
classDiagram
    ...
    class FogOfWarController {
        ...
        +Start()
        +Update()
        note: 매 프레임 플레이어 위치와<br/>Y좌표 제한선을 계산해<br/>쉐이더(Material)에 전달
    }
    ...
    FogOfWarController ..> Material : 2. 데이터 쓰기 (_PlayerScreenPos, _FogYLimit)
...
sequenceDiagram
    ...
    rect rgb(200, 220, 255)
        ...
        Controller->>Controller: 1. 플레이어 화면 좌표 계산<br/>2. Y제한선 화면 좌표 변환
        Controller->>Material: SetVector("_PlayerScreenPos")
        Controller->>Material: SetFloat("_FogYLimit")
    end
    ...
    rect rgb(255, 220, 200)
        ...
        opt Pass가 등록된 경우
            ...
            Pass->>Material: GetFloat("_FogYLimit")
            Pass->>GameLoop: Blit(Source -> Temp -> Dest)
            note right of Pass: 쉐이더: Y > Limit 이면<br/>안개 적용 안 함
        end
    end
```
